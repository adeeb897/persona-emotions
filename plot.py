...
def fig_accuracy_vs_distance(plt, probe, axis, layer):
    ...
    for pid in [p["id"] for p in personas.PERSONAS]:
        if pid not in distance:
            continue
        if pid == default:
            acc, sd = per_layer["acc_default_heldout"], per_layer["acc_default_heldout_sd"]
        else:
    ...
    for is_control, colour, marker, label in (
        ...
        if not sel:
            continue
        ax.errorbar(
            [p[1] for p in sel], [p[2] for p in sel], yerr=[p[3] for p in sel],
            fmt="none", ecolor=colour, elinewidth=1.4, capsize=3,
            capthick=1.4, alpha=0.7, zorder=2,
        )
        ax.scatter(
            [p[1] for p in sel], [p[2] for p in sel],
            s=90 if not is_control else 80,
            c=colour, marker=marker, zorder=3,
            edgecolors=SURFACE, linewidths=2,   # 2px surface ring, not a border
            label=label,
        )
    ...
    for pid, x, acc, sd, _ in sorted(points, key=lambda p: p[1]):
        for dy in (0.035, -0.050, 0.080, -0.095, 0.125):
            ...
            fx, fy = frac(x, anchor)
            if not any(abs(fx - px) < 0.15 and abs((fy + dy) - py) < 0.038
                       for px, py in placed):
                break
        placed.append((fx, fy + dy))
        ax.annotate(pid, (x, anchor), textcoords="offset points",
                    xytext=(0, dy * AXES_H_POINTS), ha="center",
                    fontsize=8.5, color=INK_SECONDARY)

    chance = probe["chance"]
    ax.axhline(chance, color=MUTED, linewidth=0.8, zorder=1)
    ax.annotate(f"chance ({chance:.2f})", (ax.get_xlim()[1], chance),
                textcoords="offset points", xytext=(-4, 5), ha="right",
                fontsize=8.5, color=MUTED)

    ax.set_xlabel(f"distance along the assistant axis "
                  f"({default} = 0, {probe.get('far_persona', axis['far_persona'])} = 1)")
    ax.set_ylabel(f"probe accuracy, held-out topics (mean of {n_folds} folds)")
    ax.set_title(f"Emotion probe transfer across personas\n"
    ...
    write_csv(
        ...
        ["persona_id", "is_control", "axis_distance", "accuracy_heldout_scenarios",
         "sd_across_folds", "sem_across_folds", "n_folds"],
        [[pid, int(ctl), f"{x:.4f}", f"{acc:.4f}", f"{sd:.4f}",
          "" if sem_of(pid) is None else f"{sem_of(pid):.4f}", n_folds]
...
def fig_confusion(plt, probe, layer):
    ...
    ax.set_ylabel("true")
    ax.set_title(f"Default-persona probe, layer {layer}\n"
...
def fig_accuracy_by_layer(plt, probe):
    ...
    ax.errorbar(layers, accs, yerr=sds, fmt="none", ecolor=SERIES_PERSONA,
                elinewidth=1.4, capsize=3, capthick=1.4, alpha=0.7, zorder=2)
    ax.plot(layers, accs, color=SERIES_PERSONA, linewidth=2, marker="o",
    ...
    ax.annotate(f"best: layer {best} ({accs[bi]:.3f})", (best, accs[bi]),
                textcoords="offset points", xytext=(0, 14), ha="center",
                fontsize=9, color=INK_SECONDARY)

    ax.set_xlabel("layer")
    ax.set_ylabel(f"accuracy, held-out topics (mean of {n_folds} folds)")
    ax.set_title("Default-persona probe accuracy by layer\n"
    ...
    write_csv(config.FIG_DIR / "accuracy_by_layer.csv",
              ["layer", "acc_default_heldout", "sd_across_folds", "n_folds"],
              [[l, f"{a:.4f}", f"{s:.4f}", n_folds]
               for l, a, s in zip(layers, accs, sds)])
...
def main() -> int:
    ...
    for path, stage in ((config.PROBE_RESULTS_JSON, "probes.py"),
